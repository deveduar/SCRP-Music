...
function vercelLikeReq(req: IncomingMessage): Record<string, unknown> {
...
function vercelLikeRes(res: ServerResponse): Record<string, unknown> {
  const shim = Object.create(res) as Record<string, unknown>
  shim.status = (code: number) => {
    res.statusCode = code
    return shim
  }
  shim.send = (body: unknown) => {
